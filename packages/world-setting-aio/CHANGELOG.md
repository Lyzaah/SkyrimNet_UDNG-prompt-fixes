...
### Fixed
- A speaker whose own hands are bound (armbinder, elbow tie, butterfly
  binder, yoke, straitjacket, arm cuffs, or bondage mittens) could still be
  instructed to `BDSMLOCK` a device onto someone else, or narrate the
  behind-the-back "Arms first" restraint — `_speakerHandsBusy` existed but
  only ever gated the dressing/stripping branch, never the actual lock menu.
  Both `native_action_selector.prompt` and `0760_bdsm_lock_action.prompt`
  (Hard and Medium narrative components alike) now short-circuit the whole
  BDSMLOCK/BDSMUNLOCK/Vibe*/change_outfit_target section when the speaker's
  own hands are busy, and the native selector's eligible-actions list drops
  those actions from the menu entirely in that case, not just the
  instruction text.
